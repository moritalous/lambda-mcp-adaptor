...
export class MCPServer {
  ...
  validateWithZod(zodSchema, args) {
    ...
    for (const [key, schema] of Object.entries(zodSchema)) {
      try {
        if (args[key] === undefined && this.isZodOptional(schema)) {
  ...
  async handleInitialize(params) {
    ...
    return {
      ...
      capabilities: {
        ...
        resources: this.resources.size > 0 ? { listChanged: true } : undefined,
 ...
 */
export function createLambdaHandler(mcpServer) {
  return async (event, context) => {
    try {
      ...
      if (method === 'GET') {
        return createErrorResponse(405, -32000, 'Method not allowed: Stateless mode', corsHeaders);
      }
      
      return createErrorResponse(405, -32000, `Method not allowed: ${method}`, corsHeaders);
...
async function handleMCPRequest(mcpServer, body, headers, corsHeaders) {
  ...
  try {
    ...
    let errorCode = -32603;
    if (error.message.includes('not found')) errorCode = -32601;
    else if (error.message.includes('Validation error')) errorCode = -32602;
...
/**
 * Utility function to create a new MCP server
 */
